...
| 2 | セージ | #33B679 | rgb(51, 182, 121) | **通常予約（X DM）** |
...
| 7 | ピーコック | #039BE5 | rgb(3, 155, 229) | 対面予約（ブロック判定用） |
| 8 | グラファイト | #616161 | rgb(97, 97, 97) | - |
| 9 | ブルーベリー | #3F51B5 | rgb(63, 81, 181) | - |
| 10 | バジル | #0B8043 | rgb(11, 128, 67) | - |
| 11 | トマト | #D50000 | rgb(213, 0, 0) | **LINE連携予約（PersonA）** |

### このプロジェクトでの色分けルール

- **colorId '2' (トマト・赤)**: PersonA（LINE連携）からの予約
- **colorId '7' (ピーコック・青)**: 通常リンクからの予約（X DM経由）
- **colorId '11' (セージ・緑)**: PersonC（LINE連携）からの予約
...
```javascript
{
  ...
  colorId: "2"（PersonA）/ "11"（PersonC）/ "7"（通常）,
  reminders: {
    useDefault: false,
    overrides: [
