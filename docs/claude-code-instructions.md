...
変更後の確認は以下の基準で使い分けること。

### ローカル確認優先
以下の変更はデプロイ前に `npm start` でローカル確認してからデプロイする：
- ビジュアル変更（フォントサイズ・色・レイアウト・余白など）
- UIコンポーネントの表示ロジック変更
- フロントエンドのロジック変更（ブロック判定など）

### デプロイ優先
以下の変更はローカルでは確認できないためデプロイして本番確認する：
- Netlify Functions（バックエンド）の変更
- 外部API連携（Google Calendar・ChatWork・LINE）の変更
- 環境変数に依存する機能の変更
