...
**アクセスURL**:
- 通常: `https://mfagencybooking.netlify.app/` → X DM経由（タグなし）
- PersonA: `https://mfagencybooking.netlify.app/personA` → LINE連携（公認LINE）
- PersonB: `https://mfagencybooking.netlify.app/personB` → X DM経由（まゆ紹介タグ）
- PersonC: `https://mfagencybooking.netlify.app/personC` → LINE連携（まえかぶLINE）

### 💾 入力キャッシュ（通常フロー）
- X経由（通常フロー）でお名前・Xリンク・myfans/P登録状況を入力して次へ進むと、localStorageに自動保存
- 次回アクセス時に入力済み状態でフォームが開く（同端末・同ブラウザ）
- LINEフローには非対応（LINE側にユーザー情報が紐づくため不要）
...
- システムエラー時にChatWork（`CHATWORK_ROOM_ID`）へ自動アラート（全Functionに適用）
  - 予約作成失敗・LINE通知失敗・コールバックエラー・カレンダー取得失敗・削除失敗
  - 共通アラート関数を `netlify/functions/shared/chatwork.js` に集約
- 前日リマインド実行結果のChatWork報告は無効化済み（有効化する場合は `scheduled-reminder.js` の `sendChatWorkReminderReport` 呼び出しのコメントを外す）
- データ検証機能
...
### 🔐 LINE連携機能（実装済み）
- **LIFF（LINE Front-end Framework）方式**で認証（外部ブラウザ対応）
  - PersonA（公認LINE）: LIFF ID = `REACT_APP_LIFF_ID`
  - PersonC（まえかぶLINE）: LIFF ID = `REACT_APP_LIFF_ID_C`
  - 認証後 `line-session-create.js` でセッションをGoogleカレンダーに保存
- プロフィール情報の自動取得（`liff.getProfile()`）
...
- カレンダーの場所欄: PersonA = `公式LINE（公認）` / PersonC = `公式LINE（まえかぶ）`

**Netlify環境変数（全量）:**
- `GOOGLE_CALENDAR_ID` / `GOOGLE_SERVICE_ACCOUNT_KEY`
- `CHATWORK_API_TOKEN` / `CHATWORK_ROOM_ID` / `CHATWORK_BOOKING_ROOM_ID`
- `LINE_CHANNEL_ACCESS_TOKEN`（PersonA通知用）/ `LINE_CHANNEL_ACCESS_TOKEN_C`（PersonC通知用）
- `LINE_CHANNEL_ID` / `LINE_CHANNEL_SECRET`（PersonA認証用）
- `LINE_CHANNEL_ID_C` / `LINE_CHANNEL_SECRET_C`（PersonC認証用）
- `REACT_APP_LINE_CHANNEL_ID`（PersonA）/ `REACT_APP_LINE_CHANNEL_ID_C`（PersonC）※netlify.tomlにも記載必要
- `REACT_APP_LIFF_ID`（PersonA）/ `REACT_APP_LIFF_ID_C`（PersonC）※netlify.tomlにも記載必要
- `REACT_APP_TEST_USER_ID` / `REACT_APP_TEST_USER_PW`
...
│       ├── line-callback.js            # LINE OAuth コールバック（PersonA・旧フロー）
│       ├── line-callback-c.js          # LINE OAuth コールバック（PersonC・旧フロー）
│       ├── line-session-create.js      # LIFFログイン後のセッション保存API
...
CHATWORK_ROOM_ID=your_chatwork_room_id               # システムアラート用ルーム
CHATWORK_BOOKING_ROOM_ID=your_chatwork_booking_room_id  # 予約完了通知用ルーム

# LINE Messaging API（通知用）
# PersonA（公認LINE）
LINE_CHANNEL_ACCESS_TOKEN=your_line_channel_access_token
# PersonC（まえかぶLINE）
LINE_CHANNEL_ACCESS_TOKEN_C=your_line_channel_access_token_c
...
# PersonC（まえかぶLINE）
LINE_CHANNEL_ID_C=your_line_login_channel_id_c
LINE_CHANNEL_SECRET_C=your_line_login_channel_secret_c

# React アプリ用（フロントエンド）
REACT_APP_LINE_CHANNEL_ID=your_line_login_channel_id       # PersonA
REACT_APP_LINE_CHANNEL_ID_C=your_line_login_channel_id_c   # PersonC
...
### 1. LINE Developersコンソールでチャネルを作成（PersonA・PersonC各2チャネル）

#### LINE Messaging API チャネル（通知用）
1. プロバイダーを選択 → 新規チャネル作成 → **Messaging API**
2. チャネル基本設定から以下を取得：
   - PersonA: **Channel Access Token** → `LINE_CHANNEL_ACCESS_TOKEN`
   - PersonC: **Channel Access Token** → `LINE_CHANNEL_ACCESS_TOKEN_C`

#### LINE Login チャネル（認証用）
1. プロバイダーを選択 → 新規チャネル作成 → **LINE Login**
2. チャネル基本設定から以下を取得：
   - PersonA: **Channel ID** → `LINE_CHANNEL_ID` / `REACT_APP_LINE_CHANNEL_ID`、**Channel Secret** → `LINE_CHANNEL_SECRET`
   - PersonC: **Channel ID** → `LINE_CHANNEL_ID_C` / `REACT_APP_LINE_CHANNEL_ID_C`、**Channel Secret** → `LINE_CHANNEL_SECRET_C`
3. LINE Login設定：
   - PersonA **コールバックURL**: `https://mfagencybooking.netlify.app/.netlify/functions/line-callback`
   - PersonC **コールバックURL**: `https://mfagencybooking.netlify.app/.netlify/functions/line-callback-c`
   - **OpenID Connect**: 有効化
   - **公開設定**: 開発中（テスターを追加）または公開済み

### 2. Netlifyに環境変数を設定
- Site settings → Environment variables → Add a variable
- 上記で取得した値を設定（PersonA・PersonC両方）

### 3. netlify.toml に環境変数を追加
```toml
[build.environment]
  REACT_APP_LINE_CHANNEL_ID = "your_line_channel_id"
  REACT_APP_LINE_CHANNEL_ID_C = "your_line_channel_id_c"
```
