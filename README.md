...
- 画面左上に表示されていたデバッグ用ステータス（`Initial/TimeSlots/BookingForm/ConfirmScreen/Confirmation`）を削除
- Googleカレンダーの `colorId` を使ったブロック判定を強化
  - `colorId: '7'`（ピーコック）を「対面」扱いとして前後3時間ブロック
  - `colorId: '3'`（グレープ）を「撮影」扱いとして当日 + 後3時間ブロック
- クエリAPIのレスポンスに `colorId` を含め、フロント判定とサーバー最終チェックの整合性を改善
