...
- 前日リマインド実行結果を毎日15時にChatWork報告（cronの動作確認用）
- データ検証機能
...
- LINE Login OAuth 2.0 認証
  - PersonA（公認LINE）: `line-callback.js` / コールバック: `/.netlify/functions/line-callback`
  - PersonC（まえかぶLINE）: `line-callback-c.js` / コールバック: `/.netlify/functions/line-callback-c`
- プロフィール情報の自動取得
